...
class LMCSCModel:
    def __init__(
        ...
        alpha: float = 1.0,  # hyperparameter for the length reward
        beta: float = 0.5,  # hyperparameter for the length reward
    ...
    def decorate_model_instance(self) -> None:
        ...
        self.model.decode = self.tokenizer.decode
        self.tokenizer.padding_side = "left"


        # vocab_size = max(len(self.vocab.items()), self.model.vocab_size)
        ...
        # print("字典大小", self.model.vocab_size)
        self.model.align_vocab = LLM_to_TraditionalModel_Vocab(
            self.vocab,
            is_bytes_level=self.is_byte_level_tokenize,
        ...
        self.model.token_ids_cache = {}

        self.model.alpha = self.alpha
        self.model.beta = self.beta
        self.model.is_bytes_level = self.is_byte_level_tokenize
        ...
        self.model.small_model_to_probs = (
            small_model_to_probs.__get__(self.model, type(self.model))
        )
        self.model.get_distortion_probs = get_distortion_probs.__get__(
            self.model, type(self.model)
        )
        self.model.get_small_model_probs = get_small_model_probs.__get__(
            self.model, type(self.model)
        )
    ...
    def __call__(self, src: List[str], sentence_id, prob_distribute, contexts: List[str] = None):
        (
            model_kwargs,
            context_input_ids,
            context_attention_mask,
        ...
        with torch.no_grad():
            outputs = self.model.distortion_guided_beam_search(
                ...
                align_vocab=self.model.align_vocab.align_vocab,
                **model_kwargs,
            )
