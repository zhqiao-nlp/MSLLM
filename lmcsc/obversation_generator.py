...
class NextObversationGenerator(BaseObversationGenerator):
    ...
    def get_steps(self) -> List[str]:
        ...
        for batch_idx, steps in enumerate(self.batch_steps):
            ...
            cur_steps.append(observed_steps)
        return cur_steps
    
    def get_true_steps(self) -> List[str]:
        cur_steps = []
        temp = deepcopy(self.batch_steps)
        # for i in range(len(temp[0])):
        #     try:
        #         temp[0][i] = temp[0][i].decode("utf-8")
        #     except:
        #         temp[0][i] = ""
        #         self.batch_steps[0][i] = ""
        for batch_idx, steps in enumerate(temp):
            observed_steps = []
            for beam_idx in range(len(steps)):
                try:
                    steps[beam_idx] = steps[beam_idx].decode("utf-8")
                except:
                    steps[beam_idx] = ""
                    # self.batch_steps[batch_idx][beam_idx] = ""
                step = len(steps[beam_idx])
                observed_steps.append(step)
            cur_steps.append(observed_steps)
        return cur_steps
